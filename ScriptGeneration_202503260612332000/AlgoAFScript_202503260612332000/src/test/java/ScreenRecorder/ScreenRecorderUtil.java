...
public class ScreenRecorderUtil extends ScreenRecorder {
 ...
 protected File createMovieFile(Format fileFormat) throws IOException {
  ...
  File ScreenRecFileName = new File(movieFolder,
		    name + "-" + dateFormat.format(new Date()) + "." + Registry.getInstance().getExtension(fileFormat));
  System.out.println("  ScreenRecFileName =    "+ScreenRecFileName);
  
  return ScreenRecFileName;
//  return new File(movieFolder,
 //   name + "-" + dateFormat.format(new Date()) + "." + Registry.getInstance().getExtension(fileFormat));
 }

 public static void startRecord(String methodName) throws Exception {
  file = new File("./Video-recordings/");
 ...
 */
       public static void deleteRecordedFile() {
             if (file != null) {
                   ...
                   new File(file, children[i-1]).delete();
                   System.out.println("Deleted this " + file + "\\"+ children[i-1]);
                  // new File(file, children[i-1]).delete();
