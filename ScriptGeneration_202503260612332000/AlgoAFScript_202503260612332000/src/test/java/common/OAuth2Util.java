...
public class OAuth2Util {
	static final Logger log = Logger.getLogger(OAuth2Util.class);
	...
	public static String getAccessTokenfromOAuthAPI(String text) {
		...
		try {
			...
			String clientauthentication = obj.has("client_authentication")
					? obj.get("client_authentication").getAsString()
					: "";
			...
			String granttype = obj.has("grant_type") ? obj.get("grant_type").getAsString() : "";
			...
			if (granttype.equals("authorization_code")) {
				...
				if (userBrowser) {
					...
					Response response = RestAssured.given().formParam("grant_type", "password")
							.formParam("client_id", clientId).formParam("client_secret", clientSecret)
							.formParam("username", username).formParam("password", password).post(accessTokenUrl);
					accessToken = response.jsonPath().getString("access_token");
					ExtentCucumberAdapter.addTestStepLog("access token is: " + accessToken);
				}
				...
				RequestSpecification requestSpec = RestAssured.given().contentType("application/x-www-form-urlencoded")
